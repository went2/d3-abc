...
function nodeOver(_, d) {
  ...
  d3.selectAll("path").classed("active", function (p) {
    return p.source === d.source || p.target === d;
  });
}
function edgeOver(_, d) {
  ...
  d3.selectAll("circle")
    .classed("source", function (p) {
      return p === d.source;
    })
    .classed("target", function (p) {
      return p === d.target;
    });
}
function recover() {
...
function loadCsvAndDraw(urlArr, drawFunc) {
  var promises = urlArr.map(function (urlString) {
    return d3.csv(urlString);
  });
